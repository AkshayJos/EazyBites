...
const EditProfile = () => {
        ...
        if (userData.signupType !== "Food Seller") {
          setAuthState({
            ...
            authError: {
              ...
              returnPath:
                userData.signupType === "Foodie" ? "/foodie-edit-profile" : "/",
              returnText:
                userData.signupType === "Foodie"
                  ? "Go to Foodie Profile"
                  : "Go to Home",
            },
            userData,
          });
  ...
  const handleProfilePhotoUpload = async (file) => {
    ...
    try {
      ...
      setFormData((prev) => ({
        ...prev,
        photoURL: data, // assuming backend returns image URL directly
      }));
  ...
  const handleStallPhotoUpload = async (e) => {
    ...
    if (formData.stallPhotos.length + files.length > MAX_STALL_PHOTOS) {
      setError(
        `Maximum ${MAX_STALL_PHOTOS} ${
          vendorType === "shop" ? "Cafe" : "Stall"
        } photos allowed`
      );
      return;
    }
    ...
    for (const file of files) {
      ...
      try {
        ...
        setFormData((prev) => ({
          ...prev,
          stallPhotos: [...prev.stallPhotos, data], // assuming `data` is the image URL
        }));
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="Edit-FoodSeller-form">
        <div className="Edit-FoodSeller-photo-section">
          <div
            className={`Edit-FoodSeller-photo-container ${
          ...
          <div className="Edit-FoodSeller-stall-photos">
            {formData.stallPhotos.map((photo, index) => (
              ...
              >
                <img
                  src={photo}
                  alt={`{stallOrCafe} ${index + 1}`}
                  className="Edit-FoodSeller-stall-photo"
                />
                <button
                  type="button"
