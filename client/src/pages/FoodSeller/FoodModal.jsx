...
const FoodModal = ({ isOpen, onClose, onSubmit, foodItem, mode = "add" }) => {
  ...
  const uploadPhotos = async () => {
    const uploadPromises = selectedPhotos.map(async (photo) => {
      // If photo is already uploaded (e.g. during edit), return its preview URL
      if (photo.isExisting) return photo.preview;

      const formData = new FormData();
      formData.append("name", photo.file.name);
      ...
      try {
        ...
        setUploadProgress((prev) => {
          const updated = { ...prev };
          delete updated[photo.file.name];
          return updated;
        });

        return data; // assuming 'data' is the image URL
      } catch (error) {
      ...
      {isOpen && (
        ...
        >
          ...
          >
            ...
            {error && <div className="MyMenu-error-message">{error}</div>}
